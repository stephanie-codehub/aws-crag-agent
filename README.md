...
* **AI Orchestration:** LangChain, LangGraph, Groq, sentence-transformers/all-MiniLM-L6-v2
...
*   **Bounded Retry Logic:** Self-correction loops for retrieval and output guardrail checks are capped at a maximum retry count, failing gracefully with a structured error response rather than looping indefinitely. [In Progress]
*   **Scope Guardrail:** Out-of-scope queries are routed to a hardcoded response before reaching any LLM call, avoiding unnecessary token spend and latency on requests the agent isn't designed to handle. [Completed ✅️]
*   **Sandboxed Code Validation:** Provisioning code generated by the agent is validated in an isolated LocalStack sandbox before execution, catching runtime errors which are then fed back into the retrieval loop for self-correction within retry limits. [In Progress]
